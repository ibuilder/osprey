...
BUCKET_LABEL = {"act_today": "ACT TODAY", "this_week": "THIS WEEK", "watch": "WATCH", "done": "DONE"}


def _styles() -> dict[str, ParagraphStyle]:
    base = getSampleStyleSheet()
    return {
        "title": ParagraphStyle("t", parent=base["Title"], textColor=INK, fontSize=22, spaceAfter=2, alignment=TA_LEFT),
        "tag": ParagraphStyle("tag", parent=base["Normal"], textColor=MUTED, fontSize=9, italic=True, spaceAfter=8),
        ...
        "cellsm": ParagraphStyle("cs", parent=base["Normal"], fontSize=7, leading=9, textColor=MUTED),
...
def hotlist_to_pdf(payload: dict[str, Any], *, project_name: str = "Project") -> bytes:
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"Osprey Hotlist — {project_name}", author="Osprey",
    )
    flow: list = []
    ...
    table = Table(
        data,
        colWidths=[0.3 * inch, 0.75 * inch, 2.9 * inch, 0.75 * inch, 0.7 * inch, 2.0 * inch, 0.5 * inch],
        repeatRows=1,
    )
